...
class Base
{
    ...
    const SECRET = '01c6d59a3f9024db6336662ac95c8e74';
    ...
    public function getOpenId(){
        if(isset($_SESSION['openid'])){
            ...
            if(!isset($_GET['code'])){
                $redurl = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'].'index.php';
                //测试的链接 会调用微信的jsdk不走程序
//                 $redurl = 'http://paysdk.weixin.qq.com/jsapi.php';
    ...
    //调用统一下单api
    public function unifiedOrder($soid,$total_fee){
        /**
         * 1.根据数据构建原始数据
         * 2.加入签名
         * 3.将数据转换为XML
        ...
        $params = [
            ...
            //notify_url是微信回调的链接
            'notify_url'=> 'http://doc.fnying.com/api/wx_pay_syc/notify.php',
            'trade_type'=>'JSAPI',
            'product_id'=>$soid,
    ...
    //获取prepayid
    public function getPrepayId($soid,$total_fee){
        $arr = $this->unifiedOrder($soid ,$total_fee);
